...
# sim=NULL;minN=1;plot.it=TRUE;violin=.05
...
# 
# data=hsamples_Young;sim=pp2_Young;n.intervals=6
...
plot_SS_if.dmc <- function(data,sim=NULL,minN=1,plot.it=TRUE,violin=.05,ci=95,len=.05,
  ...
  data.pch=16,sim.pch=1,sim.lty=3,data.lty=1,add=FALSE,xlim=NULL)
  ...
  # NB6: Other options similar to SSRT plot
{
  ...
  if ( !fit & plot.it ) { # Inhibition Function
    ...
    lines(xvals,pR,lwd=2,lty=data.lty)
  }
  ...
  if ( fit ) {
    ...
    if ( plot.it ) { # Inhibition Function
      ...
      if (!sim.line) lines(xvals,pR,lwd=2,lty=data.lty) else
